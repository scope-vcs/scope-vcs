...
mod tests {
    ...
    fn endpoint_discovery_distinguishes_no_checkout_from_broken_config() {
        let outside = crate::test_support::TempDir::new("endpoint-outside");
        assert!(
            scope_api_url_from_git_config(outside.path())
                .unwrap()
                .is_none()
        );
        let dir = crate::test_support::TempDir::git_repo("endpoint-config", "main");
        ...
        let bare = crate::test_support::TempDir::new("endpoint-bare");
