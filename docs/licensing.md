...
Pagent is explicitly excluded from Scope's Apache-2.0 grant. The bundled
`web/vendor/pagent-v0.1.0-47b796c.tgz` remains unchanged and supplies no separate
license declaration or terms. `web/vendor/PAGENT-LICENSING.txt` records this
exception for that exact archive. It does not establish redistribution rights.
The dependency inventory and web notices preserve this unresolved licensing
status; a passing freshness check does not mean Pagent has been cleared for
reuse. A different archive requires a new review of its terms.
