...
where
    C: ConnectionTrait,
{
    let now_unix = u64_to_i64(now_unix, "request ref cleanup time")?;
...
impl CleanupStore {
            ...
            query = query
                .filter(
                    entities::request_ref_cleanup_job::Column::NextRunAtUnix
                        .lte(u64_to_i64(now, "request ref cleanup due time")?),
                )
                .limit(100);
        }
        query
    ...
    pub async fn retry_request_ref_cleanup(
        ...
        entities::request_ref_cleanup_job::Entity::update_many()
            ...
            .col_expr(
                entities::request_ref_cleanup_job::Column::NextRunAtUnix,
                sea_orm::sea_query::Expr::value(u64_to_i64(
                    now_unix.saturating_add(retry_after),
                    "request ref cleanup retry time",
                )?),
            )
            .col_expr(
