...
async fn fresh_database_reaches_exact_latest_schema() {
    ...
    let runner_stop_columns = db
        .query_one(Statement::from_string(
            ...
            "SELECT count(*) AS count
             ...
             WHERE table_schema = current_schema()
               ...
               AND column_name IN ('runner_stop_claimed_at_unix', 'runner_stop_completed_at_unix')"
                .to_string(),
        ))
        .await
        .unwrap()
        ...
        .try_get::<i64>("", "count")
        .unwrap();
    assert_eq!(runner_stop_columns, 2);
    assert!(relation_exists(db.as_ref(), "scope_request_claims").await);
    assert!(relation_exists(db.as_ref(), "scope_request_attention_states").await);
