...
fn dispatching_attempt_sql(attempt_id: &str, number: u32, token: char) -> String {
    format!(
        "INSERT INTO scope_run_attempts (
            id, run_id, job_key, number, token_hash, token_expires_at_unix, state,
            lease_expires_at_unix, last_heartbeat_at_unix, created_at_unix, log_bytes,
            runtime_version
         ) VALUES (
            '{attempt_id}', 'run-1', 'build', {number}, repeat('{token}', 64), 100,
...
#[tokio::test]
async fn m0046_rejects_a_second_dispatching_attempt_for_one_job() {
    let (_target, db, _lease) = isolated_database().await;
    // Baseline plus m0043 only: the indexes still name the impossible 'leased' state.
    migrations::Migrator::up(db.as_ref(), Some(2))
        .await
        .unwrap();
    db.execute_unprepared(FIXTURE_SQL).await.unwrap();
    db.execute_unprepared(&dispatching_attempt_sql("attempt-1", 1, 'a'))
        .await
        .unwrap();
    // The bug: nothing stopped a second in-flight attempt for the same job.
    db.execute_unprepared(&dispatching_attempt_sql("attempt-2", 2, 'b'))
        .await
        .unwrap();
    ...
    let error = db
        .execute_unprepared(&dispatching_attempt_sql("attempt-2", 2, 'b'))
        .await
        .unwrap_err()
        .to_string();
