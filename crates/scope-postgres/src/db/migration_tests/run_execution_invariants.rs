...
const SEED: &str = r#"
    ...
    INSERT INTO scope_run_attempts (id,run_id,job_key,number,token_hash,state,
        ...
        VALUES ('attempt','run','build',100,repeat('e',64),'lost',20,10,10,20,
...
async fn seed_before_execution_repair(db: &DatabaseConnection) {
    migrations::Migrator::up(db, Some(7)).await.unwrap();
