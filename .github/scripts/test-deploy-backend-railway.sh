...
printf '%s\n' test-source-sha > "$test_dir/media/.scope-deployment-sha"
...
install -m 0755 "$root/.github/scripts/test-backend-cutover-fake-railway.sh" \
  "$test_dir/bin/railway"
...
run_cutover() {
  ...
  if [[ -z "$successful_deployments" ]]; then
    successful_deployments='{"api":{"sourceSha":"test-source-sha","provider":"railway","evidenceId":"old-scope-api"},"worker":{"sourceSha":"test-source-sha","provider":"railway","evidenceId":"old-scope-worker"},"cache":{"sourceSha":"test-source-sha","provider":"railway","evidenceId":"old-scope-cache-service"},"media":{"sourceSha":"test-source-sha","provider":"railway","evidenceId":"old-scope-media"},"mediaWorker":{"sourceSha":"test-source-sha","provider":"railway","evidenceId":"old-scope-media-worker","artifactDigest":"sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"},"router":{"sourceSha":"test-source-sha","provider":"railway","evidenceId":"old-scope-repo-router"}}'
  fi
...
[[ "$(cat "$test_dir/interrupted-result")" == "0" ]]
assert_evidence_components interrupted cache,worker,media,mediaWorker,api
