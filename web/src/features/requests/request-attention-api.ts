...
import { ApiRouteTemplates, buildApiPath, type RequestAttentionActionRequest } from '@/api/types.generated'
...
export function parseRequestAttentionInput(input: unknown): RequestAttentionInput {
  ...
  const body = data.action === 'snooze'
    ? { action: data.action, expected_activity_version: data.expected_activity_version, until_unix: data.until_unix }
    : { action: data.action, expected_activity_version: data.expected_activity_version }
  if (!apiValidators.RequestAttentionActionRequest(body)) throw new Error('Request attention action is invalid.')
  return { ...params, ...body }
}
