  ...
  return (
    <ul className="m-0 list-none p-0">
      {files.map((name) => {
        ...
        return (
          <li key={name} className={cn('scene-file relative flex h-8 items-center gap-[9px] rounded-[3px] font-mono text-(length:--marketing-figure-body) [line-height:normal]', changed && (merged ? 'merged-file' : 'submitted-file'))}>
            <Icon className="icon size-[15px] text-muted-foreground" />
            {name}
            {(changed || privateFile) && (
              <span className={cn('ml-auto flex items-center gap-1.5 text-(length:--marketing-figure-meta)', privateFile ? 'text-muted-foreground' : 'text-success-strong')}>
    ...
    >
      <div aria-hidden className="request-progress mb-4 grid grid-cols-3 gap-3.5 max-[521px]:mb-4 max-[521px]:gap-2.5">
        {['Submitted', 'Your review', 'Merged'].map((step) => (
          <div key={step} className="request-step relative border-b border-border pb-2 text-(length:--marketing-figure-title) font-medium text-muted-foreground">{step}</div>
        ))}
      </div>
      <div aria-hidden className="request-sheet relative border-y border-border bg-card [--marketing-request-inset:18px] max-[521px]:[--marketing-request-inset:12px]">
        <div className="flex min-h-[52px] items-center gap-3 border-b border-border py-3 px-[var(--marketing-request-inset)] max-[521px]:min-h-[56px] max-[521px]:gap-2">
          ...
          <h3 className="m-0 min-w-0 font-sans text-(length:--marketing-figure-title) font-medium leading-[1.35] tracking-normal max-[521px]:max-w-[195px] max-[361px]:max-w-[175px]">Handle empty responses</h3>
          <div className="relative ml-auto h-5 w-[9ch] shrink-0 text-right font-mono text-(length:--marketing-figure-meta) leading-5 whitespace-nowrap text-muted-foreground *:absolute *:inset-0">
        ...
        <div className="relative h-[240px] max-[521px]:h-[260px] overflow-hidden *:pointer-events-none *:absolute *:inset-0 *:pt-4 *:px-[var(--marketing-request-inset)] *:pb-4 *:will-change-[opacity,transform] max-[521px]:*:pt-4">
          <div className="submission-scene">
            <div className="mb-[15px] flex items-center gap-2 text-(length:--marketing-figure-title) font-medium leading-5">
              <Globe className="icon size-[15px] text-muted-foreground" />
              Public clone
            </div>
            <ContributionTree />
            <div className="absolute inset-x-[var(--marketing-request-inset)] bottom-[21px] flex items-center gap-2 text-(length:--marketing-figure-meta) text-muted-foreground">
              <Check className="icon size-3.5 text-success-strong" />
              Submitted to your review queue
            </div>
          </div>
          <div className="review-scene">
            <div className="mb-[13px] flex items-center justify-between gap-4 font-mono text-(length:--marketing-figure-body) [line-height:normal]">
              <span className="flex items-center gap-2">
                <File className="icon size-3.5 text-muted-foreground" />
                sdk/api.ts
              </span>
              <span className="text-(length:--marketing-figure-meta) text-success-strong">+1 line</span>
            </div>
            <div className="review-code m-0 font-mono text-(length:--marketing-figure-body) leading-[1.9] text-muted-foreground *:block *:px-2 *:whitespace-pre-wrap *:[overflow-wrap:anywhere] max-[521px]:leading-[1.8] max-[361px]:*:px-[5px]">
            ...
            <div className="maintainer-review mt-4 grid grid-cols-[26px_minmax(0,1fr)] gap-2.5 max-[521px]:mt-4">
              ...
              <div>
                <div className="text-(length:--marketing-figure-meta) leading-[1.4] font-medium">Maintainer</div>
                <p className="mt-1 text-(length:--marketing-figure-body) leading-normal text-muted-foreground">
                  Empty responses return null. Looks good.
                </p>
              </div>
            </div>
            <div className="review-decision absolute bottom-5 left-[calc(var(--marketing-request-inset)+36px)] flex items-center gap-[7px] text-(length:--marketing-figure-meta) text-success-strong max-[521px]:bottom-[18px]">
          ...
          <div className="merged-scene">
            <div className="mb-[15px] flex items-center gap-2 text-(length:--marketing-figure-title) font-medium leading-5">
              ...
              Your repository
              <span className="ml-auto font-mono text-(length:--marketing-figure-meta) font-normal text-muted-foreground [line-height:normal]">main</span>
            </div>
