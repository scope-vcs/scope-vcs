...
const sectionCopy = 'mt-4 max-w-[380px] text-[15px] leading-[1.7] text-muted-foreground max-[901px]:max-w-[540px]'
...
export function MarketingLandingPage({
  ...
  return (
    <div className="marketing-page min-h-dvh bg-background font-sans text-base leading-normal text-foreground antialiased scheme-light dark:scheme-dark">
      <a className="fixed top-2.5 left-2.5 z-10 -translate-y-[160%] bg-foreground px-4 py-2.5 text-background focus:translate-y-0" href="#main-content">Skip to content</a>
      <header className={cn('topbar flex h-[76px] items-center justify-between max-[521px]:h-[72px]', shell)}>
        <div className="flex items-center gap-[17px] max-[521px]:gap-3">
          ...
          <span className="border-l border-border pl-[17px] font-mono text-xs leading-[normal] text-muted-foreground max-[521px]:pl-3 max-[521px]:text-[10px] max-[361px]:hidden">pre-alpha</span>
        </div>
        <nav className="flex items-center gap-7 max-[521px]:gap-3 [&_button]:size-[38px] [&_button]:rounded-full [&_button]:bg-transparent [&_button:hover]:bg-muted" aria-label="Account and appearance">
          <Link
            className="text-[14px] hover:text-success-strong max-[521px]:text-[13px]"
            params={{ _splat: '' }}
            search={{ redirect_url: '/' }}
            to="/sign-in/$"
          >
      ...
      <main className={cn(shell, 'outline-none [--marketing-section-space:104px] max-[901px]:[--marketing-section-space:80px]')} data-scope-page="landing" id="main-content" tabIndex={-1}>
        <section className={cn('pt-16 max-[901px]:gap-9 max-[901px]:pt-10 max-[521px]:pt-8', sectionLayout)} aria-labelledby="hero-title">
          <div className="min-w-0 max-[901px]:max-w-[640px]">
            <h1 id="hero-title" className="enter text-[clamp(42px,4vw,56px)] leading-[1.06] font-[500] tracking-[-.05em] max-[901px]:text-[48px] max-[521px]:text-[clamp(34px,8.7vw,42px)]">
              ...
              <span className={cn(headlineLine, 'text-muted-foreground')}>You choose</span>{' '}
              <span className={cn(headlineLine, 'text-muted-foreground')}>what’s <span className="accent relative inline-block text-success-strong after:absolute after:right-0 after:-bottom-[5px] after:left-px after:h-0.5 after:origin-left after:bg-success after:content-['']">public.</span></span>
            </h1>
            <p className="enter mt-5 max-w-[390px] text-base leading-[1.65] text-muted-foreground [animation-delay:90ms] max-[901px]:max-w-[540px] max-[521px]:text-[15px]">
            ...
            <div className="enter mt-6 flex flex-wrap items-center gap-x-5 gap-y-2 [animation-delay:180ms] max-[521px]:gap-x-4">
              <a href="#install" className="group inline-flex min-h-11 items-center justify-center gap-3 rounded-[5px] bg-foreground px-4 py-2.5 text-[13px] font-medium text-background transition-[transform,box-shadow] duration-180 hover:-translate-y-0.5 hover:shadow-[0_4px_0_var(--border)] max-[521px]:gap-3.5 max-[521px]:px-3.5">
                Install Scope<ArrowDown className="icon transition-transform duration-200 group-hover:translate-y-0.5" aria-hidden />
              </a>
              <a href={sourceUrl} className="group inline-flex items-center gap-2 py-2.5 text-[14px] hover:text-success-strong max-[521px]:gap-[5px] max-[521px]:text-[13px]">
        ...
        <section className={cn('contributions pt-[var(--marketing-section-space)] max-[901px]:gap-8', sectionLayout)} aria-labelledby="contribution-title">
          <div>
        ...
        <section className={cn('install scroll-mt-8 pt-[var(--marketing-section-space)] pb-12 max-[901px]:gap-8', sectionLayout)} id="install" aria-labelledby="install-title">
          <div>
      ...
      <footer className={cn('footer flex min-h-[90px] items-center justify-between gap-5 text-xs leading-normal text-muted-foreground max-[521px]:flex-col max-[521px]:items-start max-[521px]:justify-center max-[521px]:gap-[17px] max-[521px]:py-[25px] [&_a:hover]:text-success-strong', shell)}>
