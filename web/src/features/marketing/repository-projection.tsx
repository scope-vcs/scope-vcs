...
function VisibilityLabel({ isPublic }: { isPublic: boolean }): ReactElement {
  ...
  return (
    <span className={`ml-auto inline-flex items-center justify-end text-xs ${isPublic ? 'is-public text-success-strong' : 'is-private text-muted-foreground'}`}>
...
function RepositoryColumn({ publicClone = false }: { publicClone?: boolean }): ReactElement {
  ...
  return (
    <div className={cn('min-w-0', publicClone && 'border-l border-border bg-muted')}>
      <div className="flex h-11 items-center gap-[9px] border-b border-border px-5 text-[13px] font-medium max-[1151px]:gap-[7px] max-[1151px]:px-4 max-[521px]:gap-1.5 max-[521px]:px-[11px] max-[361px]:gap-1 max-[361px]:px-[7px]">
        ...
        {files.map((name) => {
          ...
          return (
            <li
              ...
              className={cn(
                'repository-file relative flex h-[38px] min-w-0 items-center gap-[9px] rounded-sm px-1.5 font-mono text-[13px] [line-height:normal] max-[1151px]:gap-[7px] max-[521px]:gap-1.5 max-[521px]:px-[5px] max-[361px]:gap-1 max-[361px]:px-1',
...
export function RepositoryProjection(): ReactElement {
  return (
    <div className="min-w-0 w-full">
      ...
      >
        <div aria-hidden className="flex h-11 items-center justify-between gap-4 border-b border-border px-5 font-mono text-[13px] font-medium [line-height:normal] max-[1151px]:px-4 max-[521px]:px-3.5">
