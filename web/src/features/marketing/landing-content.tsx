...
const section = 'relative grid grid-cols-[minmax(0,.8fr)_minmax(0,1.2fr)] items-start gap-18 pt-[var(--marketing-section-space)] max-[901px]:grid-cols-1 max-[901px]:gap-12'
const sectionTitle = 'max-w-[12ch] text-[clamp(32px,3.6vw,48px)] leading-[1.04] font-medium tracking-[-.045em]'
const sectionNote = 'absolute left-0 top-[calc(100%+28px)] max-w-[32ch] max-[901px]:static max-[901px]:mt-5 max-[901px]:block'
  ...
  return (
    <LandingViewProvider view={view}>
      <div className={landingShell}>
        ...
        <main className="outline-none [--marketing-section-space:144px] max-[901px]:[--marketing-section-space:96px]" id={isPublic ? 'main-content' : undefined} tabIndex={isPublic ? -1 : undefined}>
          <section className="relative grid min-h-[calc(100dvh-76px)] grid-cols-[minmax(0,1fr)_400px] items-center gap-24 pb-24 max-[901px]:min-h-0 max-[901px]:grid-cols-1 max-[901px]:gap-14 max-[901px]:pt-28 max-[421px]:pt-40">
          ...
          <section className={section} id={isPublic ? 'merge' : undefined}>
            <div className="relative">
              <Heading className={sectionTitle} level={2}><Swap text={mergeTitle} /></Heading>
              <Note className={sectionNote} id="merge" />
            </div>
            <MergeGraph />
            <Note className="absolute right-0 top-[calc(100%+24px)] max-w-[24ch] text-right" id="corner" />
          </section>

          <section className={cn(section, 'scroll-mt-8 pb-12 max-[901px]:pb-36')} id={isPublic ? 'install' : undefined}>
            <div className="relative">
              <Heading className={sectionTitle} level={2}><Swap text={installTitle} /></Heading>
              <Note className={sectionNote} id="install" />
            </div>
            <div className="min-w-0" data-note="command">
              <CliInstallCommand
        ...
        <footer className="relative flex min-h-24 items-center justify-between gap-5 text-[13px] text-muted-foreground">
          <Link aria-label="Scope home" to="/"><ScopeLogo className="landing-logo w-[84px] opacity-65" /></Link>
          <Note className="absolute left-1/2 top-1/2 max-w-[44ch] -translate-x-1/2 -translate-y-1/2 text-center max-[901px]:top-auto max-[901px]:bottom-[calc(100%+16px)] max-[901px]:translate-y-0" id="footer" />
