...
test('update file selection follows explicit links, close, reselect and browser Back', async () => {
  const updatePath = `${repoPath}/updates/dev-public-1`
  await withPage(updatePath, async (page) => {
    ...
    await page.getByText('Select a changed file', { exact: true }).waitFor()
    await page.goto(`${baseUrl}${updatePath}?path=%2Fsrc%2Fapp.ts`)
    await readyDiff(page, 'src/app.ts')
    await page.goBack()
    await page.getByText('Select a changed file', { exact: true }).waitFor()
    await page.evaluate((to) => globalThis.__TSR_ROUTER__.navigate({
      to, search: { path: '/README.html' },
    }), updatePath)
    await readyDiff(page, 'README.html')
    await page.getByRole('button', { name: 'app.ts', exact: true }).click()
    await readyDiff(page, 'src/app.ts')
    await page.setViewportSize({ width: 320, height: 800 })
    const toggle = page.getByRole('button', { name: /^files src\/app.ts$/ })
    await toggle.click()
    await page.getByRole('button', { name: 'README.html', exact: true }).click()
    await readyDiff(page, 'README.html')
