...
use crate::{
    error::ApiError,
    git::{
        command::{remaining_git_time, run_git_output_until, truncated_git_stderr},
        import::git_changed_tree_entries_until,
    },
};
...
pub(crate) fn inspect_native_public_commit(
    ...
    let new_entries = git_changed_tree_entries_until(repo, first_parent, &native.oid, deadline)?;
    let mut old_entries = match first_parent {
        Some(parent) => git_changed_tree_entries_until(repo, Some(&native.oid), parent, deadline)?
            .into_iter()
