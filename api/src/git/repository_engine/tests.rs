...
fn test_engine(test: &str) -> Arc<RepositoryEngine> {
    ...
    RepositoryEngine::for_tests(root, 1024 * 1024 * 1024).unwrap()
}
...
fn separate_engines_do_not_reuse_a_predecessor_incarnation_cache() {
    ...
    let recreated_engine = RepositoryEngine::for_tests(root.clone(), 1024 * 1024 * 1024).unwrap();
...
async fn cancelled_derived_build_and_active_readers_remain_leased_during_eviction() {
    ...
    let engine = RepositoryEngine::for_tests(root.path().to_path_buf(), 1).unwrap();
