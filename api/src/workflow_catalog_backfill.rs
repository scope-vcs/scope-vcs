...
impl AppState {
    ...
    pub async fn backfill_repository_workflow_catalogs(&self) -> anyhow::Result<usize> {
        backfill_repository_workflow_catalogs(&RepositoryBackfillContext::from_app_state(self))
            .await
    }
}

pub async fn backfill_repository_workflow_catalogs_for_maintenance(
    database_url: String,
) -> anyhow::Result<usize> {
    let context = RepositoryBackfillContext::from_env(database_url).await?;
    backfill_repository_workflow_catalogs(&context).await
}

async fn backfill_repository_workflow_catalogs(
