...
pub async fn backfill_git_segments_v2_for_maintenance(
    ...
    let segment_store = segment_store_from_env(local_root, encryption_key)?;
...
async fn legacy_store_from_env(encryption_key: [u8; 32]) -> anyhow::Result<Arc<dyn ObjectStore>> {
    ...
    if crate::dev::is_local_dev_env() {
        let root = data_dir(&crate::config::git_repo_root()).join("objects");
...
fn segment_store_from_env(
    ...
    if crate::dev::is_local_dev_env() {
        let root = data_dir(&crate::config::git_repo_root()).join("objects");
        return Ok(GitSegmentStore::new(
            Arc::new(object_store_config::git_segment_file_store_from_env(&root)?),
            scope_git_storage::SegmentEncryptionKey::new("primary", encryption_key)?,
            crate::config::git_segment_store_config_from_env(local_root)?,
        )?);
    }
    object_store_config::git_segment_store_from_env(local_root, encryption_key)
}
